...
class RatingDonutView @JvmOverloads constructor(context: Context, attributeSet: AttributeSet? = null) : View(context, attributeSet) {
    ...
    // Метод для выбора цвета краски, т.к. цвет будет меняться от уровня рейтинга
    private fun getPaintColor(progress: Int): Int = when(progress) {
        in 0 .. 25 -> Color.parseColor("#FD0808")
        in 26 .. 50 -> Color.parseColor("#FF8000")
        in 51 .. 75 -> Color.parseColor("#FDE508")
        else -> Color.parseColor("#39FD08")
    }
    ...
    // Метод для отрисовки арок, т.к. прогресс от 0 до 100, а круг 360 градусов, поэтому надо конвертировать эти значения
    ...
    // Метод для отрисовки нашего кастомного View
    ...
    // Метод, чтобы задать рейтинг из кода
